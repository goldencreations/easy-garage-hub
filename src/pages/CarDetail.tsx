...
import { carDetailsRequest, type InvoiceApi } from "@/lib/api";
...
export default function CarDetail() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null);
      try {
        const response = await carDetailsRequest(token, id);
        setPayload(response.data);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Could not load car details.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const createPdfBlob = useCallback(
    async (invoice: InvoiceApi, regeneratedAt?: Date) => {
      if (!car) return new Blob([], { type: "application/pdf" });
      ...
      const mockCustomer: MockCustomer = {
        id: String(owner?.id ?? car.customer_id),
        name: owner?.name ?? "Unknown customer",
        phone: owner?.phone ?? "—",
        email: owner?.email ?? "—",
        address: owner?.address ?? "Dar es Salaam",
        carIds: [String(car.id)],
        createdAt: "",
      };
  ...
  if (!car) return <p className="text-sm text-muted-foreground">No car details found.</p>;
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        description={`${car.vehicle_type} • ${car.model_year} • ${car.color}`}
      />

      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <DataCard title="Vehicle Info">
          <div className="space-y-2 p-5 text-sm">
            <p><span className="text-muted-foreground">Plate:</span> <span className="font-mono font-semibold">{car.plate_number}</span></p>
            <p><span className="text-muted-foreground">Vehicle:</span> {car.vehicle_type}</p>
            <p><span className="text-muted-foreground">Year:</span> {car.model_year}</p>
            <p><span className="text-muted-foreground">Color:</span> {car.color}</p>
          </div>
        </DataCard>

        <DataCard title="Owner">
          <div className="space-y-2 p-5 text-sm">
            <p className="flex items-center gap-2"><User className="h-4 w-4 text-muted-foreground" /> {owner?.name ?? "—"}</p>
            ...
            <p><span className="text-muted-foreground">Address:</span> {owner?.address ?? "—"}</p>
          </div>
        </DataCard>

        <DataCard title="Summary">
          <div className="space-y-2 p-5 text-sm">
            <p className="flex justify-between"><span>Services</span><span className="font-semibold">{payload?.history?.services_count ?? services.length}</span></p>
            <p className="flex justify-between"><span>Invoices</span><span className="font-semibold">{payload?.history?.invoices_count ?? invoices.length}</span></p>
            <p className="flex justify-between"><span>Paid</span><span className="font-semibold">{formatCurrency(totals.paid)}</span></p>
            <p className="flex justify-between"><span>Outstanding</span><span className="font-semibold">{formatCurrency(totals.unpaid)}</span></p>
          </div>
        </DataCard>
      </div>
